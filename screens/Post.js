...
import { addPost, addProfile, Clear } from "../redux/authSlice";
import {
  AntDesign,
  EvilIcons,
...
export const Post = (props) => {
  ...
  const success_addImgToPost = async () => {
    SetDetails("");
    SetTitle("");
    setIsRain(false);
    setIsWinter(false);
    setIsSummer(false);
    setImages([]);
    setMyLongtitude(13.121151);
    setMyLatitude(100.91723);

    context = await { ...context, img_name: img_name };
    context = await { ...context, id: getnewPostID };

    await dispatch(addPost(context));
  ...
  const success_img = (url) => {
    img_name.push(url);
    if (img_name.length === images.length) {
      addImgToPost();
    }
  };
  const post_todb = () => {
    ...
    <View style={{ flex: 1 }}>
      <ScrollView style={{ backgroundColor: "#F9F9F9" }}>
        ...
        <SelectDropdown
          ...
          selectedRowTextStyle={{ color: "red", fontWeight: "bold" }}
          dropdownStyle={{ borderRadius: 25 }}
          onSelect={(selectedItem, index) => {
            console.log(selectedItem, index);
            setSelectprovince(selectedItem);
          ...
          defaultButtonText="โปรดเลือกจังหวัด"
        />
        ...
        >
          ...
          <Text>โปรดแตะค้างที่หมุดเพื่อยกหมุดขึ้น</Text>
        </View>
        {/* Upgrade Return End */}
        <View>
          <View
        ...
        {title && details && isProvinceSelected && images.length != 0 ? (
          <TouchableOpacity
          ...
          <TouchableOpacity
            ...
            onPress={() => {
              Alert.alert("โปรดใส่ หัวข้อ, ข้อมูล, จังหวัด และรูปภาพให้ครบ");
            }}
          >
            <Text
              style={{ textAlign: "center", fontWeight: "bold", fontSize: 20 }}

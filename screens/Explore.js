...
export const Explore = (props) => {
  ...
  useEffect(() => {
    ...
    dataPresort = [...data.post];

    dataPresort.sort(function (a, b) {
      if (a.date.today === b.date.today) {
        // Price is only important when cities are the same
        return b.date.today - a.date.today;
      }
      return a.date.today > b.date.today ? -1 : 1;
    });
    setPost(dataPresort);
  }, []);
  ...
  const [post, setPost] = useState(useSelector((state) => state.auths.post));
  ...
  const Item = (props) => {
    const redux = useSelector((state) => state.auths.post);
    ...
    const likeing = () => {
      ...
      dispatch(editStar({ postid: data.id, myid: myid }));
    };
    const like_database_process = (id) => {
      ...
      if (my_like==true) {
        ...
        // if (mes.length == 0) {
        //   console.log("เขา length ==0")
        //   mes.push(myid);
        //   setArrStar(mes);
        //   console.log(arrstar,myid)
        //   PostModel.update_like(id, mes, likeing, unsuccess);
        // }else{
